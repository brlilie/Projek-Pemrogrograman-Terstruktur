#include <iostream>
#include <vector>
#include <list>
#include <string>
#include <algorithm>
#include <array>
#include <unordered_map>
#include <fstream>
#include <sstream>
#include <set>
#include <map>

using namespace std;

// Kelas untuk merepresentasikan tiket
class Ticket {
public:
    string movie;
    vector<string> seats;
    string day;
    string time;
    double price;

    Ticket(string m, vector<string> s, string d, string t, double p) : movie(m), seats(s), day(d), time(t), price(p) {}
};

// Fungsi untuk mengubah string menjadi huruf kecil
string toLowerCase(string str) {
    transform(str.begin(), str.end(), str.begin(), ::tolower);
    return str;
}

// Fungsi untuk membagi array menjadi dua bagian (divide and conquer)
void merge(vector<Ticket*>& tickets, int l, int m, int r) {
    int n1 = m - l + 1;
    int n2 = r - m;

    vector<Ticket*> L(n1);
    vector<Ticket*> R(n2);

    for (int i = 0; i < n1; i++)
        L[i] = tickets[l + i];
    for (int i = 0; i < n2; i++)
        R[i] = tickets[m + 1 + i];

    int i = 0, j = 0, k = l;

    while (i < n1 && j < n2) {
        if (L[i]->price <= R[j]->price) {
            tickets[k] = L[i];
            i++;
        } else {
            tickets[k] = R[j];
            j++;
        }
        k++;
    }

    while (i < n1) {
        tickets[k] = L[i];
        i++;
        k++;
    }

    while (j < n2) {
        tickets[k] = R[j];
        j++;
        k++;
    }
}

// Fungsi untuk melakukan merge sort (divide and conquer)
void mergeSort(vector<Ticket*>& tickets, int l, int r) {
    if (l < r) {
        int m = l + (r - l) / 2;
        mergeSort(tickets, l, m);
        mergeSort(tickets, m + 1, r);
        merge(tickets, l, m, r);
    }
}

// Fungsi untuk menampilkan tiket
void displayTickets(const vector<Ticket*>& tickets) {
    cout << "\n--- Daftar Tiket ---\n";
    for (const auto& ticket : tickets) {
        cout << "Film: " << ticket->movie << ", Kursi: ";
        for (const auto& seat : ticket->seats) {
            cout << seat << " ";
        }
        cout << ", Hari: " << ticket->day << ", Jam: " << ticket->time 
             << ", Harga: Rp" << ticket->price * ticket->seats.size() << endl; 
    }
    cout << "--------------------\n";
}

// Fungsi untuk mendapatkan harga tiket berdasarkan hari dan jenis kursi
double getTicketPrice(const string& seat, const string& day) {
    bool isWeekend = (day == "sabtu" || day == "minggu");
    bool isHotSeat = (seat[0] == 'v');  // Cek kursi hotseat dengan huruf awal 'v'

    if (isWeekend) {
        return isHotSeat ? 60000 : 50000;
    } else {
        return isHotSeat ? 40000 : 35000;
    }
}

// Fungsi untuk menampilkan informasi harga dan status kursi
void displayPriceInfo(const list<string>& movies, const map<string, bool>& bookedSeats) {
    array<string, 2> seatTypes = {"regular", "hotseat"};
    cout << "\n--- Informasi Harga Tiket ---\n";
    for (const auto& movie : movies) {
        for (const auto& seatType : seatTypes) {
            cout << "Film: " << movie << ", Kursi: " << seatType << ", Hari: Weekday, Harga: Rp" << getTicketPrice(seatType == "hotseat" ? "v1" : "h1", "senin") << endl;
            cout << "Film: " << movie << ", Kursi: " << seatType << ", Hari: Weekend, Harga: Rp" << getTicketPrice(seatType == "hotseat" ? "v1" : "h1", "sabtu") << endl;
        }
    }

    cout << "\n--- Status Kursi ---\n";
    for (const auto& [seat, isBooked] : bookedSeats) {
        cout << "Kursi " << seat << " - " << (isBooked ? "Sudah Dipesan" : "Tersedia") << endl;
    }
    cout << "-----------------------------\n";
}

// Fungsi untuk memesan tiket
bool bookTicket(vector<Ticket*>& tickets, const list<string>& movies, map<string, bool>& bookedSeats) {
    string movie, day, time, promoCode;
    int numTickets;
    vector<string> chosenSeats;
    bool validInput;

    cout << "\nFilm yang tersedia: ";
    for (const auto& m : movies) {
        cout << "[" << m << "] ";
    }
    cout << endl;

    do {
        cout << "Pilih film: ";
        cin >> movie;
        movie = toLowerCase(movie);
        validInput = find(movies.begin(), movies.end(), movie) != movies.end();
        if (!validInput) {
            cout << "Input yang anda masukkan salah. Silakan coba lagi.\n";
        }
    } while (!validInput);

    cout << "Hari yang tersedia: Senin, Selasa, Rabu, Kamis, Jumat, Sabtu, Minggu" << endl;
    do {
        cout << "Pilih hari: ";
        cin >> day;
        day = toLowerCase(day);
        validInput = (day == "senin" || day == "selasa" || day == "rabu" || day == "kamis" || day == "jumat" || day == "sabtu" || day == "minggu");
        if (!validInput) {
            cout << "Input yang anda masukkan salah. Silakan coba lagi.\n";
        }
    } while (!validInput);

    cout << "Jam yang tersedia: 08:00 sampai 21:00" << endl;
    do {
        cout << "Pilih jam: ";
        cin >> time;
        validInput = (time >= "08:00" && time <= "21:00");
        if (!validInput) {
            cout << "Jam yang anda masukkan salah. Silakan coba lagi.\n";
        }
    } while (!validInput);

    cout << "Berapa tiket yang ingin dipesan: ";
    cin >> numTickets;

    for (int i = 0; i < numTickets; ++i) {
        string seatType;
        cout << "Kursi yang tersedia: Regular, Hotseat" << endl;
        do {
            cout << "Pilih jenis kursi untuk tiket " << i + 1 << ": ";
            cin >> seatType;
            seatType = toLowerCase(seatType);
            validInput = (seatType == "regular" || seatType == "hotseat");
            if (!validInput) {
                cout << "Input yang anda masukkan salah. Silakan coba lagi.\n";
            }
        } while (!validInput);

        string seat;
        do {
            if (seatType == "regular") {
                cout << "Pilih kursi (h1 - h30): ";
            } else {
                cout << "Pilih kursi (v1 - v10): ";
            }
            cin >> seat;
            seat = toLowerCase(seat);
            validInput = (seatType == "regular" && seat[0] == 'h' && stoi(seat.substr(1)) >= 1 && stoi(seat.substr(1)) <= 30) ||
                         (seatType == "hotseat" && seat[0] == 'v' && stoi(seat.substr(1)) >= 1 && stoi(seat.substr(1)) <= 10);
            if (validInput && bookedSeats[seat]) {
                cout << "Kursi " << seat << " sudah dipesan. Silakan pilih kursi lain.\n";
                validInput = false;
            }
            if (!validInput) {
                cout << "Input yang anda masukkan salah. Silakan coba lagi.\n";
            }
        } while (!validInput);

        chosenSeats.push_back(seat);
    }

    double ticketPrice = getTicketPrice(chosenSeats[0], day);
    double totalPrice = ticketPrice * numTickets;
    double payment;

    // Memasukkan kode promo
    cout << "Masukkan kode promo (opsional, tekan enter jika tidak ada): ";
    cin.ignore();
    getline(cin, promoCode);
    promoCode = toLowerCase(promoCode);

    // Jika kode promo valid, terapkan diskon 2%
    if (promoCode == "nontonyuk") {
        totalPrice *= 0.98;  // Diskon 2%
        cout << "Kode promo diterapkan. Total harga setelah diskon: Rp" << totalPrice << endl;
    }

    cout << "Total harga: Rp" << totalPrice << endl;
    cout << "Masukkan pembayaran: Rp";
    cin >> payment;

    if (payment < totalPrice) {
        cout << "Pembayaran tidak mencukupi. Transaksi dibatalkan.\n";
        return false;
    }

    // Tandai kursi sebagai sudah dipesan
    for (const auto& seat : chosenSeats) {
        bookedSeats[seat] = true;
    }

    double change = payment - totalPrice;
    cout << "Kembalian: Rp" << change << endl;

    // Buat tiket baru dan tambahkan ke daftar tiket
    Ticket* ticket = new Ticket(movie, chosenSeats, day, time, ticketPrice);
    tickets.push_back(ticket);

    // Tampilkan struk tiket
    cout << "\n--- Struk Tiket ---\n";
    cout << "Film: " << movie << "\nHari: " << day << "\nJam: " << time << "\nKursi: ";
    for (const auto& seat : chosenSeats) {
        cout << seat << " ";
    }
    cout << "\nTotal Harga: Rp" << totalPrice << "\n--------------------\n";

    return true;
}

// Fungsi untuk refund tiket
void refundTicket(vector<Ticket*>& tickets, map<string, bool>& bookedSeats) {
    string movie, day, time;
    string seat;
    double refundAmount = 0.0;
    bool validInput;
    auto it = tickets.begin();

    do {
        cout << "Film: ";
        cin >> movie;
        movie = toLowerCase(movie);
        validInput = any_of(tickets.begin(), tickets.end(), [&](Ticket* t) { return t->movie == movie; });
        if (!validInput) {
            cout << "Input yang anda masukkan salah. Silakan coba lagi.\n";
        }
    } while (!validInput);

    do {
        cout << "Hari: ";
        cin >> day;
        day = toLowerCase(day);
        validInput = any_of(tickets.begin(), tickets.end(), [&](Ticket* t) { return t->day == day; });
        if (!validInput) {
            cout << "Input yang anda masukkan salah. Silakan coba lagi.\n";
        }
    } while (!validInput);

    do {
        cout << "Jam: ";
        cin >> time;
        validInput = (time >= "08:00" && time <= "21:00");
    } while (!validInput);

    do {
        cout << "Kursi: ";
        cin >> seat;
        seat = toLowerCase(seat);
        validInput = any_of(tickets.begin(), tickets.end(), [&](Ticket* t) { return find(t->seats.begin(), t->seats.end(), seat) != t->seats.end(); });
        if (!validInput) {
            cout << "Input yang anda masukkan salah. Silakan coba lagi.\n";
        }
    } while (!validInput);

    while (it != tickets.end()) {
        if ((*it)->movie == movie && find((*it)->seats.begin(), (*it)->seats.end(), seat) != (*it)->seats.end() && (*it)->day == day && (*it)->time == time) {
            refundAmount += (*it)->price;
            bookedSeats[seat] = false;  // Hapus kursi dari set kursi yang sudah dipesan
            delete *it;
            it = tickets.erase(it);
        } else {
            ++it;
        }
    }

    if (refundAmount > 0) {
        cout << "Tiket berhasil direfund. Jumlah refund: Rp" << refundAmount << endl;
    } else {
        cout << "Tiket tidak ditemukan atau tidak valid.\n";
    }
}

// Fungsi untuk memeriksa login pengguna
bool userLogin() {
    unordered_map<string, string> users;
    ifstream userFile("users.txt");

    if (userFile.is_open()) {
        string username, password;
        while (userFile >> username >> password) {
            users[username] = password;
        }
        userFile.close();
    }

    string inputUsername, inputPassword;
    cout << "Masukkan username: ";
    cin >> inputUsername;
    cout << "Masukkan password: ";
    cin >> inputPassword;

    auto it = users.find(inputUsername);
    if (it != users.end() && it->second == inputPassword) {
        cout << "Login berhasil. Selamat datang, " << inputUsername << "!" << endl;
        return true;
    } else {
        cout << "Username atau password salah. Silakan coba lagi.\n";
        return false;
    }
}

// Fungsi untuk registrasi pengguna baru
void registerUser() {
    unordered_map<string, string> users;
    ifstream userFile("users.txt");

    if (userFile.is_open()) {
        string username, password;
        while (userFile >> username >> password) {
            users[username] = password;
        }
        userFile.close();
    }

    string newUsername, newPassword;
    cout << "Masukkan username baru: ";
    cin >> newUsername;

    if (users.find(newUsername) != users.end()) {
        cout << "Username sudah ada. Silakan pilih username lain.\n";
        return;
    }

    cout << "Masukkan password baru: ";
    cin >> newPassword;

    users[newUsername] = newPassword;

    ofstream outFile("users.txt");
    for (const auto& user : users) {
        outFile << user.first << " " << user.second << endl;
    }
    outFile.close();

    cout << "Registrasi berhasil. Silakan login untuk melanjutkan.\n";
}

// Fungsi untuk reset password
void forgotPassword() {
    unordered_map<string, string> users;
    ifstream userFile("users.txt");

    if (userFile.is_open()) {
        string username, password;
        while (userFile >> username >> password) {
            users[username] = password;
        }
        userFile.close();
    }

    string username, newPassword;
    cout << "Masukkan username Anda: ";
    cin >> username;

    auto it = users.find(username);
    if (it != users.end()) {
        cout << "Masukkan password baru: ";
        cin >> newPassword;
        users[username] = newPassword;

        ofstream outFile("users.txt");
        for (const auto& user : users) {
            outFile << user.first << " " << user.second << endl;
        }
        outFile.close();

        cout << "Password berhasil direset. Silakan login dengan password baru.\n";
    } else {
        cout << "Username tidak ditemukan.\n";
    }
}

// Fungsi utama
int main() {
    vector<Ticket*> tickets;
    list<string> movies = {"avengers", "inception", "interstellar"};
    map<string, bool> bookedSeats;

    // Inisialisasi semua kursi sebagai tersedia (false)
    for (int i = 1; i <= 30; ++i) {
        bookedSeats["h" + to_string(i)] = false;
    }
    for (int i = 1; i <= 10; ++i) {
        bookedSeats["v" + to_string(i)] = false;
    }

    int choice;
    bool exit = false;

    while (true) {
        cout << "\n****************************\n";
        cout << "*        TICKET SHOP       *\n";
        cout << "****************************\n";
        cout << "1. Login\n";
        cout << "2. Registrasi\n";
        cout << "3. Lupa Password\n";
        cout << "4. Keluar\n";
        cout << "****************************\n";
        cout << "Masukkan pilihan Anda: ";
        cin >> choice;

        if (choice == 1 && userLogin()) {
            break;
        } else if (choice == 2) {
            registerUser();
        } else if (choice == 3) {
            forgotPassword();
        } else if (choice == 4) {
            cout << "Terima kasih telah menggunakan Ticket Shop. Sampai jumpa lagi!\n";
            return 0;
        } else {
            cout << "Input yang anda masukkan salah. Silakan coba lagi.\n";
        }
    }

    while (!exit) {
        cout << "\n****************************\n";
        cout << "*        TICKET SHOP       *\n";
        cout << "****************************\n";
        cout << "1. Informasi Harga\n";
        cout << "2. Pesan Tiket\n";
        cout << "3. Tampilkan Tiket\n";
        cout << "4. Urutkan Tiket Berdasarkan Harga\n";
        cout << "5. Refund Tiket\n";
        cout << "6. Keluar\n";
        cout << "****************************\n";
        cout << "Masukkan pilihan Anda: ";
        cin >> choice;

        switch (choice) {
            case 1:
                displayPriceInfo(movies, bookedSeats);
                break;
            case 2:
                if (bookTicket(tickets, movies, bookedSeats)) {
                    cout << "Transaksi berhasil.\n";
                }
                break;
            case 3:
                cout << "Tiket:\n";
                displayTickets(tickets);
                break;
            case 4:
                mergeSort(tickets, 0, tickets.size() - 1);
                cout << "Tiket diurutkan berdasarkan harga:\n";
                displayTickets(tickets);
                break;
            case 5:
                refundTicket(tickets, bookedSeats);
                break;
            case 6:
                exit = true;
                cout << "Terima kasih telah menggunakan Ticket Shop. Sampai jumpa lagi!\n";
                break;
            default:
                cout << "Pilihan tidak valid. Silakan coba lagi.\n";
        }
    }

    for (auto ticket : tickets) {
        delete ticket;
    }

    return 0;
}
